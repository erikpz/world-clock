...
export const WorldClock = () => {
  ...
  const handleFetch = async (e: any, o: any) => {
    
    setsearch(o)
    /*  try {
      const timeService = TimeService.getInstance();
      const response = await timeService.getTimeAreas(location);
  ...
  return (
    <ClockContainer>
      {/* <button onClick={() => handleFetch("america")}>fetch</button> */}
      <Autocomplete
        freeSolo
        autoHighlight
        disablePortal
        options={results}
        sx={{ width: 300 }}
        onHighlightChange={handleFetch}
        
        onInputChange={(e: any, newInput: string) => handleSearch(newInput)}
        renderInput={(params: any) => (
          <TextField {...params} placeholder="buskar" />
        )}
      />
    </ClockContainer>
  );
